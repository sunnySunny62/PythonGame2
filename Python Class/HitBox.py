...
class Hero:
    ...
    def draw_character(self, win):
        self.hitBox = (self.x + 10, self.y + 10, 42, 55) # So the hitbox can be draw again
        pygame.draw.rect(win, (37, 219, 210), self.hitBox, 2)
        if self.stepIndex >= 9:
            self.stepIndex = 0
        if self.face_left:
...
class Enemy():
    ...
    def draw_enemy(self, win):
        self.step()
        self.hitBox = (self.x + 15, self.y - 5, 50, 65)
        pygame.draw.rect(win, (30, 32, 148), self.hitBox, 2)
